...
HeartbeatController.HeartbeatUpdateEventEmitter
    .on('update', email_name => matcherCache.del(email_name))
    .on('delete', email_name => matcherCache.del(email_name));
...
export async function getMatcher(email_name) {
    ...
    for (const from_matcher of (matcher.from ?? [])) {
        ...
        if (!match) {
           logger.warn(`Heartbeat ${email_name} has an invalid from matcher: ${from_matcher}`);
            ...
            compiledMatcher.from.push(new RegExp(pattern, flags));
        }
    }
    ...
    for (const subject_matcher of (matcher.subject ?? [])) {
        ...
        if (!match) {
            ...
            const [, pattern, flags] = match;
            compiledMatcher.subject.push(new RegExp(pattern, flags));
        }
    }
    ...
    for (const body_matcher of (matcher.from ?? [])) {
        ...
        if (!match) {
            ...
            const [, pattern, flags] = match;
            compiledMatcher.body.push(new RegExp(pattern, flags));
        }
    }

    matcherCache.set(email_name, compiledMatcher);
